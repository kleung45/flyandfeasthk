...
## SEO／變現設定
- GTM 容器：GTM-KQVGKZTS，放 `pipeline/store.json` → `meta.gtmContainerId`，
  `build_site.py` 每次 build 自動注入 head script + body noscript。
- GA4 ID（如有）放 `meta.ga4MeasurementId`（G-XXXX…）；用戶改行 GTM 路線，
  GA4 tag 由 GTM 後台設定，此欄位目前留空。
